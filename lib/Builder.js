...
class Builder {
     ...
     */
    constructor(options) {
        /** @var {boolean} _es2018 Boolean indicating support for ES2018 features. */
	this._es2018 = options && !!options.es2018;
     ...
     * @param  {Closure|Builder|string} condition Anonymous function with its Builder as a first parameter.
     * @param  {String} name
     * @param  {boolean} es2018
     * @return {Builder}
     */
    capture(conditions, name, es2018 = false) {
	let pattern = '(%s)';
        if (name) {
            this._captureNames.push(name)
	    if (this._es2018 || es2018) {
                pattern = `(?<${name}>%s)`;
	    }
        }

        this._validateAndAddMethodType(METHOD_TYPE_GROUP, METHOD_TYPES_ALLOWED_FOR_CHARACTERS)
        return this._addClosure(new Builder()._extends(pattern), conditions)
    }
